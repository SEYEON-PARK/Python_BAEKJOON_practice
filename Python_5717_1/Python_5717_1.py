...
while True: # 무한 반복
    M, F=map(int, input().split()) # 사용자로부터 두 개의 정수(상근이의 남자 친구의 수와 여자 친구의 수) 입력받기
    if M==0 and F==0: # 만약, M(상근이의 남자 친구의 수)이 0이고 F(상근이의 여자 친구의 수)도 0이라면
       break # 가장 가까운 반복문(while문) 빠져 나가기
    print(M+F) # 결과(상근이의 친구) 출력하기
