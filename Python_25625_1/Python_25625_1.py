...
x, y=map(int, input().split()) # 사용자로부터 두 개의 정수(버스가 서울대입구역에서 제2공학관으로 이동하는 시간, 제2공학관에 버스 도착 예정 시간) 입력받기

if(y>x): # 만약, y가 x보다 크다면
    y-=x # y에 y-x의 값 대입하기
    print(y) # 결과(민준이가 버스에 탑승하기 위해서 기다려야 하는 최소 시간) 출력하기
else: # y가 x보다 크지 않다면
    print(x+y) # 결과(민준이가 버스에 탑승하기 위해서 기다려야 하는 최소 시간) 출력하기
